...
def mphash(*args):
    ...
    # log.debug("[mphash] %s" %  repr(args))
    try:
        head, tail = args[0], args[1:]
    except Exception as e:
        print(e)
        raise ValueError
    if not tail and isinstance(head, list):
...
def rad2asc(rad, s):
    ...
    for i in range(len(s)):
        ...
        if i % 3 == 0:
            s[i] = rad50[int((tmp / (40 * 40)) % 40)]
        elif i % 3 == 1:
            s[i] = rad50[int((tmp / 40) % 40)]
        elif i % 3 == 2:
            s[i] = rad50[int(tmp % 40)]
    return "".join(s)
...
def asc2rad(s):
    ...
    for i in range(0, 6):
        ...
        if (i % 3) == 2:
            res |= (int(tmp) << int((i / 3)) * 16) & 0xffffffff
            tmp = 0
    return res
